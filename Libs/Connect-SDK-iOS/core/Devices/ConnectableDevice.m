...
- (instancetype) initWithJSONObject:(NSDictionary *)dict
{
    ...
    if (self)
    {
        ...
        id lastPinged = dict[@"lastPinged"];
        if (lastPinged && ![lastPinged isKindOfClass:[NSNull class]])
            _lastPinged = [lastPinged doubleValue];
...
- (NSDictionary *) toJSONObject
{
    ...
    if (self.lastPinged) jsonObject[@"lastPinged"] = @(self.lastPinged);
...
- (BOOL) connected
{
    ...
    [_services enumerateKeysAndObjectsUsingBlock:^(id key, DeviceService *service, BOOL *stop)
    {
        if (!service.isConnectable || [service.serviceDescription.serviceId compare:kConnectSDKAirPlayServiceId] == NSOrderedSame)
            connectedCount++;
        else
        {
    ...
    if (existingService)
    {
        if (service.serviceDescription.lastDetection > existingService.serviceDescription.lastDetection)
        {
            if (existingService.connected)
                [existingService disconnect:nil];
