...
{
    _running = YES;
    _runTimer = [NSTimer scheduledTimerWithTimeInterval:15 target:self selector:@selector(checkReachability) userInfo:nil repeats:YES];
    [_runTimer fire];
}
...
//    __block float delay = (arc4random() % 100) / (float) 100;
//
//    for (NSUInteger i = 0; i < pingCount; i++) {
//        NSLog(@"PING: %@", self.targetURL.absoluteString);
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, delay * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
//            PingOperation *pingOperation = [[PingOperation alloc] initWithIPToPing:self.targetURL.absoluteString andCompletionHandler:^(NSError * error, NSString *ip) {
//                if (error) {
//                    NSLog(@"PING ERROR %@", self.targetURL.absoluteString);
//                }
//                attempt += 1;
//                if (error == nil) {
//                    isPingSuccess = YES;
...
//                            dispatch_async(dispatch_get_main_queue(), ^
//                            {
//                                NSString *ip = [NSString stringWithFormat:@"ip:%@", self.targetURL.absoluteString];
//                                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Ping error" message:ip delegate:self cancelButtonTitle:@"cancel" otherButtonTitles:@"ok", nil];
//                                [alert show];
//                                [self.delegate didLoseReachability:self];
//                            });
//                        }
