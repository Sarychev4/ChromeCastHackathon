...
//        NSURL *targetURL = [[NSURL alloc] initWithString:service.serviceDescription.address];
...
// temp vr 1           [[UIApplication sharedApplication] endBackgroundTask:self.backgroundTaskId];
//            _backgroundTaskId = UIBackgroundTaskInvalid;
//        });
//    }
...
- (void)didLoseReachability:(DeviceServiceReachability *)reachability
{
    if (self.connected)
    {
        [self.service disconnect:CONNECT_SDK_REACHABILITY_ERROR];
        [self disconnect];
    } else {
        [_serviceReachability stop];
//        [DiscoveryManager.sharedManager discoveryProvider:nil didLoseService:self.service.serviceDescription];
    }
}
...
//temp vr        if (self.backgroundTaskId == UIBackgroundTaskInvalid)
