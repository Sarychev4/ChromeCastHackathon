...
- (void) start
{
    ...
    [self.server addHandlerForMethod:@"GET" path:@"/media" requestClass:[GCDWebServerRequest class] asyncProcessBlock:^(__kindof GCDWebServerRequest *request, GCDWebServerCompletionBlock completionBlock) {
        
//        NSString *urlString = [[request.URL.absoluteString stringByReplacingOccurrencesOfString:@"media?" withString:@"media/"] stringByReplacingOccurrencesOfString:@"49291" withString:@"5000"];
//        NSURL *url = [[NSURL alloc]initWithString:urlString];
        
//        NSData *tmpImage = [[NSData alloc]initWithContentsOfURL:url];
//        GCDWebServerDataResponse *response = [GCDWebServerDataResponse responseWithData:tmpImage contentType:@"image/jpeg"];
//        completionBlock(response);
        
//        GCDWebServerDataResponse *response = [GCDWebServerDataResponse responseWithRedirect:url permanent:YES];
//        completionBlock(response);
        NSLog(@"TV TRY DOWNLOAD CONTENT: %@", request.URL.absoluteString);
//        GCDWebServerFileResponse *response = [GCDWebServerFileResponse responseWithFile:_lastFileURL.path];
//        completionBlock(response);
    }];
