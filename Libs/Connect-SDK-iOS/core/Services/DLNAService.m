...
//    NSOperationQueue *_commandQueue;
    ...
    BOOL _isReachabilityRequesting;
}
...
//    NSURL *reachURL = _reachabilityURL ? _reachabilityURL : _avTransportControlURL;
//    if (!_serviceReachability) {
//        NSURL *targetURL = [[NSURL alloc] initWithString:self.serviceDescription.address];
//        _serviceReachability = [self createDeviceServiceReachabilityWithTargetURL:targetURL];
...
- (void) updateControlURLs
{
    ...
    [serviceList enumerateObjectsUsingBlock:^(id service, NSUInteger idx, BOOL *stop) {
        ...
        NSString *reachabilityPath = service[@"SCPDURL"][@"text"];
        NSURL *controlURL = [self serviceURLForPath:controlPath];
        NSURL *eventURL = [self serviceURLForPath:eventPath];
        NSURL *reachabilityURL = [self serviceURLForPath:reachabilityPath];
...
{
    NSURL *reachURL = _reachabilityURL ? _reachabilityURL : _avTransportControlURL;
    _serviceReachability = [self createDeviceServiceReachabilityWithTargetURL:reachURL];
    _serviceReachability.delegate = self;
    [_serviceReachability start];
...
- (void) displayImageWithMediaInfo:(MediaInfo *)mediaInfo
                           ...
                           failure:(FailureBlock)failure
{ 
    NSString *url = mediaInfo.url.absoluteString;
      
    NSString *mimeType = mediaInfo.mimeType ?: @"";
    NSString *mediaInfoURLString = url ?: @"";
...
{
    NSString *url = mediaInfo.url.absoluteString;
    mediaInfo.url = [[NSURL alloc]initWithString:url];
