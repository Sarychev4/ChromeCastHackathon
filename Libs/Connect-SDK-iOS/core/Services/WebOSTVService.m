...
@interface WebOSTVService () <UIAlertViewDelegate, WebOSTVServiceSocketClientDelegate, DeviceServiceReachabilityDelegate>
{
...
- (void) launchYouTube:(NSString *)contentId startTime:(float)startTime success:(AppLaunchSuccessBlock)success failure:(FailureBlock)failure
{
    ...
    if (contentId && contentId.length > 0)
    {
        ...
        params = @{
            @"contentId" : contentId, // Этот вариант не тестил, надо проверить.
//            @"contentId" : [NSString stringWithFormat:@"https://www.youtube.com/tv?%@", contentId] // Этот вариант тоже не тестил, надо проверить.
//    Это вариант который был по-дефолту:        @"contentId" : [NSString stringWithFormat:@"%@&pairingCode=%@&t=%.1f", contentId, [[CTGuid randomGuid] stringValue], startTime]
            // Я пытался, но не помогло @"contentTarget" : [NSString stringWithFormat:@"http://www.youtube.com/tv?v=%@", contentId]
        };
    }
...
{
    if (self.webAppSessions.count > 0) {
        WebOSWebAppSession *session = self.webAppSessions[@"MediaPlayer"];
        return [session getPositionWithSuccess:success failure:failure];
    } else {
        [self sendNotSupportedFailure:failure];
    }
}
...
- (ServiceSubscription *)subscribePlayStateWithSuccess:(MediaPlayStateSuccessBlock)success failure:(FailureBlock)failure
{
    if (self.webAppSessions.count > 0) {
        WebOSWebAppSession *session = self.webAppSessions[@"MediaPlayer"];
        return [session subscribePlayStateWithSuccess:success failure:failure];
    } else {
