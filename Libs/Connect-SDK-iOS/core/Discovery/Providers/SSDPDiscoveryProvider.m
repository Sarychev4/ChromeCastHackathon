...
static double searchAttemptsBeforeKill = 2.0;
...
- (void) removeService:(ServiceDescription *)serviceDescription {
    if (serviceDescription.UUID != nil) {
        [_foundServices removeObjectForKey:serviceDescription.UUID];
        [_helloDevices removeObjectForKey:serviceDescription.UUID];
    }
}
...
//        [self sendRequestForFilter:searchFilter userAgentToken:userAgentToken killInactiveDevices:YES];
    }]; 
}
