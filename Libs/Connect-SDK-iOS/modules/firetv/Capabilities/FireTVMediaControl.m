...
- (void)onStatusChange:(MediaPlayerStatus *)status positionChangedTo:(long long)position {
    ...
    [self.mediaInfoSubscription.successCalls enumerateObjectsUsingBlock:^(id success, NSUInteger idx, BOOL *stop)
     {
        SuccessBlock mediaInfoSuccess = (SuccessBlock) success;
        if (mediaInfoSuccess) {
            NSDictionary *mediaInfo = @{@"position" : @(position)};
            mediaInfoSuccess(mediaInfo);
        }
    }];
}
