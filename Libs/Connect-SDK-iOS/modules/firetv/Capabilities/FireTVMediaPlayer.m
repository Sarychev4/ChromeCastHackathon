...
{
    SuccessBlock ifSuccessBlock = ^(id __nullable result) {
        success(0.3);
    };
    [self continueTask:[self.remoteMediaPlayer getVolume] withSuccessCompleter:^(id __nullable result) {
        success(0.3);
    } ifSuccessBlock:ifSuccessBlock andFailureBlock:failure];
}

- (void)setVolume:(float)volume success:(SuccessBlock)success failure:(FailureBlock)failure
{
    [self continueTask:[self.remoteMediaPlayer setVolume:0.5] withSuccessCompleter:^(id __nullable result) {
        success(result);
    } ifSuccessBlock:success andFailureBlock:failure];
}
