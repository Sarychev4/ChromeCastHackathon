...
@interface CastWebAppSession () <GCKUIMediaControllerDelegate>
...
- (void) connectWithSuccess:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    _castServiceChannel = [[CastServiceChannel alloc] initWithAppId:self.launchSession.appId session:self.delegate];

    // clean up old instance of channel, if it exists
//temp vr   1  [self.service.castDeviceManager removeChannel:_castServiceChannel];

    _castServiceChannel.connectionSuccess = success;
    _castServiceChannel.connectionFailure = channelFailure;

    //temp vr 1  [self.service.castDeviceManager addChannel:_castServiceChannel];
}
...
- (void)disconnectFromWebApp
{
    ...
    //temp vr  [self.service.castDeviceManager removeChannel:_castServiceChannel];
    _castServiceChannel = nil;

    //temp vr  [self.service.castDeviceManager leaveApplication];
}
...
- (void)sendText:(NSString *)message success:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    GCKError *error;
    BOOL messageSent = [_castServiceChannel sendTextMessage:message error:&error];
